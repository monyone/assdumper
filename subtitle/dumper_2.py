...
class Dumper:
  ...
  def styles(self):
    ...
    styles.append(
      self.make_style(
        ...
        Fontsize=36, # なぜかWadaLabChuMaruGo2004ARIBがデカく描写されるので
    ...
    styles.append(
      self.make_style(
        Name='ssz',
        Fontname='WadaLabChuMaruGo2004ARIB',
        Fontsize=18, # なぜかWadaLabChuMaruGo2004ARIBがデカく描写されるので
        PrimaryColour=forground_color, #固定
        SecondaryColour=forground_color, #固定
        OutlineColour=background_color,
        BackColour=background_color,
  ...
  def line_terminate(self):
    if not self.current_line or not self.current_text: return
    bg = '{{\\c&H{}&}}'.format(''.join(['{:02X}'.format(val) for val in self.bg[:-1][::-1]]))
    self.lines.append('{{\pos(0,0)}}{}{{\\p1}}m {} {} l {} {} {} {} {} {}{{\\p0}}'.format(
      bg,
      self.current_begin_pos[0], self.current_begin_pos[1],
      self.current_end_pos[0], self.current_begin_pos[1],
      self.current_end_pos[0], self.current_end_pos[1],
      self.current_begin_pos[0], self.current_end_pos[1],
    ))
  ...
  def line_changed(self):
    ...
    self.current_begin_pos = (self.pos[0], self.pos[1] - self.kukaku()[1])
    self.current_line += '{{\\pos({},{})}}'.format(self.current_begin_pos[0] + self.shs // 2, self.current_begin_pos[1] - self.svs // 2)
    self.current_text = ''
  ...
  def text_format_changed(self):
    ...
    self.current_line+= '{{\\r{}}}'.format('-'.join(params))
    self.forground_color_changed()

  def forground_color_changed(self):
    self.current_line += '{{\\c&H{}&}}'.format(''.join(['{:02X}'.format(val) for val in self.fg[:-1][::-1]])) # ffmpeg はアルファ未対応なので
  ...
  def kukaku(self):
    ...
    return (width, height)
  def move_absolute_dot(self, x, y, ignore = False):
    width, height = self.kukaku()
    self.current_end_pos = (self.pos[0] + width, self.pos[1]) if self.pos else None
    new_pos = (x, y)
    changed = not ignore and self.pos != new_pos
    self.pos = new_pos
    if changed: self.line_changed()
  def move_absolute_pos(self, x, y, ignore = False):
    width, height = self.kukaku()
    self.current_end_pos = (self.pos[0] + width, self.pos[1]) if self.pos else None
    ...
    self.pos = new_pos
    if changed: self.line_changed()
  def move_relative_pos(self, x, y, ignore = False):
    if not self.pos:
      self.move_absolute_pos(0, 0, True)
    ...
    while x < 0:
      ...
      if new_pos[0] < self.sdp[0]:
        new_pos = (self.sdp[0] + self.sdf[0] - width, new_pos[1])
        y += 1
    while x > 0:
      x -= 1
      new_pos = (new_pos[0] + width, new_pos[1])
      if new_pos[0] >= self.sdp[0] + self.sdf[0]:
        new_pos = (self.sdp[0], new_pos[1])
        y -= 1
    if y < 0:
      while y < 0:
        y += 1
    ...
    if y > 0:
      while y > 0:
        y -= 1
        new_pos = (new_pos[0], new_pos[1] + height)
    changed = not ignore and (self.pos[0] + width, self.pos[1]) != new_pos
    self.pos = new_pos
    if changed: self.line_changed()
